...
def import_events_from_sheet(file_path: str = None, events_text: str = None,
    ...
    # Check for duplicates against master discovered events using improved similarity matching
    if skip_duplicates and master_discovered_file:
        if os.path.exists(master_discovered_file):
            ...
            print(f"  Checking for duplicates against {len(master_data)} existing events...")
            ...
            for event in valid_events:
                ...
                # Import the similarity checking function
                import sys, os
                sys.path.append(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
                from stage1_event_discovery import events_are_similar
                ...
                for existing_event in master_data:
                    if events_are_similar(event, existing_event):
                        print(f"    Duplicate found: {event.get('event_name', 'Unknown')} -> {event.get('event_url', '')}")
...
IWLA Convention & Expo 2026\thttps://web.cvent.com/event/fc214f18-783d-4edf-8182-ea1e15a0d1e9/summary
